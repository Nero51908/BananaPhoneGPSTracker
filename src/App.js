...
const TELE_BOT_URL = '<API>';
const CHAT_ID = '<chatID>';// could be user input value in a pulic release
...
const LIVE_PERIOD = 300;//10800;//86400;// sec, set as 5 min for test. 24 hours for real application.
...
class Location_Cache {
    ...
    resetCache(){
      if (this.pos_hist.length > 0) {
        this.pos_hist = this.pos_hist.slice(0,1);console.log(`cached locations deleted. Home location reserved.`);
      }
      else {
        console.log(`location_cache is empty`);
      }
    }
    ...
    startLiveLocation: function(pos){
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
              ...
              // alter to setTimeout because KaiOS scheduling is not reliable.
              live_location_start = new Date().getTime();
          }
        };
    },
...
const successCall = {
    ...
    periodicCheck: function(pos) {
      ...
      let watcherMode;
      let speed = location_cache.speed();
      let distance = location_cache.dstfromHome();
      if(speed > 6){ // 6 meters/sec = 21.6 km/h
        watcherMode = "moving";console.log(`Current speed is ${speed} m/s, moving.`);
      } else {
        if(distance > 100) {
          watcherMode = "farfromHome";console.log(`Distance from home is ${distance} m, far from home`);
        } else {
          watcherMode = 'default';console.log(`Speed and distance from home are lower then threshold. Default.`);
        }
      }
      // UI update
      ...
      switch(watcherMode){
        case "moving":
          ...
          if((now - live_location_start) >= LIVE_PERIOD * 1000 || ll_message_id === 0){
        ...
        case "farfromHome":
          ...
          break;
        default:
          console.log('default branch, do nothing until the next periodic check.');
      }
    },
    live: function(pos) {
...
const softkeyCallback = { // key pressed behaviour
  ...
  right:function(){
    ...
    if(document.getElementById('softkey-right').style.color==="black") {// reaction: initialize the app
      clearInterval(watcher);watcherSet = false;console.log('periodic location update stopped (unwatched).');
      // unlock wakelock
      wakelock.unlock(); console.log('wakelock deactivated.');
      // clear cached locations but reserve home location
      location_cache.resetCache(); console.log(`history location reset.`);
  ...
  navigator.geolocation.getCurrentPosition(function(pos){
    ...
    let updateSchedule = setInterval(()=>{
      if(counter === 22){
        ...
        console.log('ll_message_id: '+ ll_message_id);
        navigator.geolocation.getCurrentPosition(successCall.live,handle_error.live,options_live);console.log('Location update (Moving):'+ counter++);
      }
    },5000);
  },
  handle_error.live,options);
